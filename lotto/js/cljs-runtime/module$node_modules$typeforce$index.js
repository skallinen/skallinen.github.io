shadow$provide.module$node_modules$typeforce$index=function(global,require,module,exports){function compile(type){if(NATIVE.String(type))return"?"===type[0]?TYPES.maybe(type.slice(1)):NATIVE[type]||TYPES.quacksLike(type);if(type&&NATIVE.Object(type)){if(NATIVE.Array(type)){if(1!==type.length)throw new TypeError("Expected compile() parameter of type Array of length 1");return TYPES.arrayOf(type[0])}return TYPES.object(type)}return NATIVE.Function(type)?type:TYPES.value(type)}function typeforce(type,
...
for(var propertyName in value){try{propertyKeyType&&typeforce(propertyKeyType,propertyName,strict)}catch(e){throw tfSubError(e,propertyName,"key");}try{typeforce(propertyType,value[propertyName],strict)}catch(e){throw tfSubError(e,propertyName);}}return!0}propertyType=compile(propertyType);propertyKeyType&&(propertyKeyType=compile(propertyKeyType));_map.toJSON=propertyKeyType?function(){return"{"+tfJSON(propertyKeyType)+": "+tfJSON(propertyType)+"}"}:function(){return"{"+tfJSON(propertyType)+"}"};
