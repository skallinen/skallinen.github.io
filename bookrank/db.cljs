...
(defn save-ranking!
  "Save the current user's ranking for a club.
   ...
   Uses merge to preserve the opinions field."
...
(defn save-opinion!
  "Save the current user's opinion for a specific book.
   Uses merge to only update the opinions map without touching order/unread."
  [club-id book-id opinion-text callback]
  (when-let [db auth/firebase-db]
    (let [uid (:uid @auth/user)]
      (-> (.set (.doc db (str "clubs/" club-id "/rankings/" uid))
                (clj->js {:opinions {(keyword book-id) opinion-text}})
                (clj->js {:merge true}))
          (.then (fn [] (when callback (callback))))
          (.catch (fn [err] (js/console.error "[db] save-opinion error:" err)))))))
